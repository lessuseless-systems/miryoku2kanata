...
kanata --check --cfg layouts/colemakdh/nix/miryoku-kanata--nix.kbd
```

### Validation with Glob Patterns

```bash
validate-miryoku-configs                       # All configs (all folders)
validate-miryoku-configs 'layouts/qwerty/**/*' # All QWERTY variants
validate-miryoku-configs 'layouts/*/mac/*'     # All macOS configs
validate-miryoku-configs 'layouts/colemakdh/nix/*' # All Colemak-DH Linux configs
```
...
3. Outputs 135 `.kbd` files organized in `layouts/<alpha>/<platform>/` folder structure
4. Enforces constraint: VI navigation is incompatible with FLIP layers (per Miryoku spec)

**Folder Organization**:
All generated configs are organized in a three-level hierarchy:
```
layouts/
  colemakdh/
    nix/        # Linux/Unix configs (5 files)
    win/        # Windows configs (5 files)
    mac/        # macOS configs (5 files)
  qwerty/
    nix/
    win/
    mac/
  ... (9 alpha layouts total)
```

This structure allows users to:
- Navigate directly to their preferred alpha layout
- Access only configs for their platform
- Keep all generated files under a single `layouts/` directory
...
Examples:
- `layouts/colemakdh/nix/miryoku-kanata--nix.kbd` → Colemak-DH, standard nav, Linux
- `layouts/qwerty/win/miryoku-kanata-qwerty-flip--win.kbd` → QWERTY, flipped layers, Windows
- `layouts/colemakdh/nix/miryoku-kanata-vi--nix.kbd` → Colemak-DH, vi nav, Linux
...
, validate 'layouts/qwerty/**/*'    # Test all QWERTY configs
, validate 'layouts/*/nix/*'        # Test all Linux configs
, validate 'layouts/colemakdh/mac/*' # Test Colemak-DH macOS configs

# Full validation
, validate  # All 135 configs across all folders
```
